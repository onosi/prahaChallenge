...
WITH RECURSIVE message_thread AS (
    -- 最初に指定したメッセージ（親メッセージ）を取得
    SELECT id, text, parentId, channelId
    FROM message
    WHERE id = 1 -- 指定されたメッセージID

    UNION ALL

    -- 親メッセージを再帰的に辿る（親メッセージを取得）
    SELECT m.id, m.text, m.parentId, m.channelId
    FROM message m
    JOIN message_thread mt ON m.id = mt.parentId
),
-- 子メッセージを再帰的に取得
child_messages AS (
    SELECT id, text, parentId, channelId
    FROM message
    WHERE parentId = 1  -- 指定されたメッセージIDを親として持つ

    UNION ALL

    SELECT m.id, m.text, m.parentId, m.channelId
    FROM message m
    JOIN child_messages cm ON m.parentId = cm.id
)
-- 最終的に親メッセージとその子メッセージを取得
SELECT id, text, parentId, channelId
FROM message_thread
UNION
SELECT id, text, parentId, channelId
FROM child_messages
ORDER BY id;  -- 結果を必要に応じて並べ替え
